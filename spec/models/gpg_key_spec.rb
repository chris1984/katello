...
module Katello
  describe ContentCredentials do
    ...
    describe "create gpg key" do
      ...
      it "should be successful with valid parameters" do
        gpg_key = ContentCredentials.new(:name => "Gpg Key 1", :content => @test_gpg_content, :organization => @organization)
        gpg_key.must_be :valid?
      end

      it 'should be destroyable' do
        gpg_key = ContentCredentials.create!(:name => "Gpg Key 1", :content => @test_gpg_content, :organization => @organization)
      ...
      it "should be unsuccessful without content" do
        gpg_key = ContentCredentials.new(:name => "Gpg Key 1", :organization => @organization)
        gpg_key.wont_be :valid?
      end

      it "should be unsuccessful without a name" do
        gpg_key = ContentCredentials.new(:content => @test_gpg_content, :organization => @organization)
        gpg_key.wont_be :valid?
      end

      it "should be unsuccessful without proper gpg key" do
        gpg_key = ContentCredentials.new(:name => "Gpg Key 1", :content => "foo-bar-baz", :organization => @organization)
      ...
      it "should be unsuccessful with binary content" do
        ...
        gpg_key = ContentCredentials.new(:name => "Gpg Key 1", :content => content, :organization => @organization)
        gpg_key.wont_be :valid?
      end
    end
  end
